...
command = "{exe} --model {mod}/BUPwithMUASimple1.nlogo --experiment {unique} --table {out}/{unique} --threads 8 > {stdo}/{unique}.output"
exe = "/share/apps/user/stow/netlogo-5.2.1/netlogo-headless.sh"
mod = "/home/wpatt2/allExp/"
out = "/home/wpatt2/NewIns_out/"
stdout = os.path.join(out, "stdout")
#command = "({}) &".format(command)

commands = []
with open("/home/wpatt2/allExp/NewIns.csv", 'r') as cfile:
    for line in cfile:
        uni = line.replace("\n", "")
        tmp_cmd = command.format(exe=exe, unique=uni, mod=mod, stdo=stdout, out=out)
...
for cabinet in range(0, 3):
    for node in range(0, 10):
        try:
            os.system("(ssh compute-{cab}-{nod} '{cmd}')&".format(cab=cabinet, nod=node, cmd=commands.pop(0)))
        except IndexError:
            pass
...
unrun_cmds = os.path.join(stdout, "EXPIREMENTS_NOT_YET_RUN.txt")
